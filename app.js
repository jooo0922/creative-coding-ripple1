...
class App {
  constructor() {
    ...
    this.radius = 10; // this.dot.animate() 메소드에 넘겨줄 radius 값
    this.pixelSize = 30; // stageWidth, Height를 각각 30으로 나눠서 각각에 들어갈 dot이 들어갈 검정색 경계선 박스의 개수를 계산하려고 할당한 값.
  ...
  drawImage() {
    ...
    // 새롭게 생성한 tmpCanvas의 2DContext인 tmpCtx에서도 브라우저 resize에 대응하여 drawImage()해 줌.
  ...
  drawDots() {
    ...
    for (let i = 0; i < this.rows; i++) {
      ...
      for (let j = 0; j < this.columns; j++) {
        ...
        // 생성한 dot 인스턴스들을 dots 배열에 차례대로 push해놓음.
        this.dots.push(dot);
      }
    }
  }
